...
public class DBUtils {
     ...
     */
        ...
        保存视频播放记录
         */
        public  void saveVideoPalyList(VideoBean bean ,String userName){
            ...
            db.insert(SQLiteHelper.U_VIDEO_PALY_LIST,null,cv);
        }
    ...
    private boolean hasVideoPlay(int chapterId, int videoId, String userName) {
        ...
        String sql = "SELECT * FROM "+SQLiteHelper.U_VIDEO_PALY_LIST+"WHERE chapterId=?" +
                " AND videoId=? AND userName=?";
    ...
    private boolean delVideoPlay(int chapterId, int videoId, String userName) {
        ...
        int row = db.delete(SQLiteHelper.U_VIDEO_PALY_LIST,"chapter=? AND userName=?",
    ...
    public List<VideoBean> getVideoHistory(String userName){
        String sql="SELECT * FEOM "+SQLiteHelper.U_VIDEO_PALY_LIST+"WHERE userName=?";
        Cursor cursor =db.rawQuery(sql,new String[]{userName});
        List<VideoBean> vbl=new ArrayList<>();
        VideoBean bean = null;
        while (cursor.moveToNext()){
            bean = new VideoBean();
            bean.chapterId = cursor.getInt(cursor.getColumnIndex("chapter"));
