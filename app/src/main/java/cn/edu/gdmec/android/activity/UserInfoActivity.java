...
public class UserInfoActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case  R.id.rl_nickName:
                ...
                bdName.putInt("flag",1);//flag传递1时表示是修改数据
                entereActivityForResult(ChangeUserInfoActivity.class,CHANGE_NICKNAME,bdName);
                //跳转到个人资料修改界面
                break;
            case R.id.rl_sex:
            ...
            case R.id.rl_signature:
                String signature = tv_signature.getText().toString();//获取个性签名控件上的数据
                Bundle bdSignature = new Bundle();
                bdSignature.putString("content",signature);//传递界面上的昵称数据
                bdSignature.putString("title","签名");
                bdSignature.putInt("flag",2);//flag传递1时表示是修改数据
                entereActivityForResult(ChangeUserInfoActivity.class,CHANGE_NICKNAME,bdSignature);
                //跳转到个人资料修改界面
                break;
                default:
                    break;
    ...
    protected  void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        /**
         * 获取回传数据时需使用的跳转方法，
         * 第一个参数to表示需要跳转到的界面,第二个参数requestCode表示一个请求码，第三个参数b表示跳转时间
         */
        public void  entereActivityForResult(Class<?> to ,int requestCode ,Bundle b){
