...
import cn.edu.gdmec.android.adapter.VideoListAdapter;
...
public class VideoListActivity extends AppCompatActivity implements View.OnClickListener{
            ...
            public void onSelect(int position, ImageView iv) {
             adapter.setSelectedPosition(position);//设置适配器的选中项
                ...
                if (TextUtils.isEmpty(videoPath)){
                    Toast.makeText(VideoListActivity.this, "此地没有此视频，暂无法播放", Toast.LENGTH_SHORT).show();
                    return;
                }else{
                    //判断用户是否登录，若登录则把此视频添加到数据库
                    if (readLoginStatus()){
                        String userName = AnalysisUtils.readLoginUserName(VideoListActivity.this);
                        db.saveVideoPalyList(videoList.get(position),userName);
                    }
                    //跳转到视频播放界面
    ...
    private String read(InputStream in){
        ...
        //用InputStreamReader把in这个字节流转化成字符流BufferedReader
